...
public class MyModel
{
    ...
    List<ChangedProperty> CreateChangedProperties(IEnumerable<string> propertyNameSet)
    {
        return [.. propertyNameSet.Select(name =>
        {
            return name switch
            {
                ...
                "ChangeTarget" => new ChangedProperty("ChangeTarget", _changeTarget),
                "RgbColor" => new ChangedProperty("RgbColor", _rgbColor),
    ...
    public void SetChangeTarget(ChangeTarget value)
    {
        EnqueueCommand(() =>
        {
            _changeTarget = value;
            NotifyToListeners("ChangeTarget");
        });
    }

    public void SetRgbColor(RgbColor value)
    {
        EnqueueCommand(() =>
        {
            _rgbColor = value;
            NotifyToListeners("RgbColor");
        });
    }
    ...
    public void ChangeNameRandomly()
    {
        EnqueueCommand(() =>
        {
            ...
            ChangeTarget target;
            if (_changeTarget == ChangeTarget.All)
                target = _rand.Next(3) switch
                {
                    0 => ChangeTarget.FirstName,
                    1 => ChangeTarget.MiddleName,
                    _ => ChangeTarget.LastName,
                };
            else
                target = _changeTarget;
            switch (target)
            {
                case ChangeTarget.FirstName:
                    SetNewName(ref _firstName, ["JAMES", "MARY", "MICHAEL"]);
                    NotifyToListeners("FirstName");
                    break;
                case ChangeTarget.MiddleName:
                    SetNewName(ref _middleName, ["", "MARIE", "ROSE", "LEE"]);
                    NotifyToListeners("MiddleName");
                    break;
                case ChangeTarget.LastName:
