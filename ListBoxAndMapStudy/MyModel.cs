...
public class MyModel
{
    ...
    public void AddFriend(string value)
    {
        EnqueueCommand(() =>
        {
            if (!_friends.TryAdd(value.ToUpper(), new FriendProperties(value.ToUpper(), "")))
            {
                // TODO エラー通知の実装
    ...
    public void RemoveFriend(string value)
    {
        EnqueueCommand(() =>
        {
            if (!_friends.Remove(value.ToUpper()))
            {
                // TODO エラー通知の実装
    ...
    public void SetActiveFirent(string? value)
    {
        EnqueueCommand(() =>
        {
            if (value == null)
            {
                ...
                NotifyToListeners(new ModelPropertyDifference.Scalar("ActiveFriendName", null));
                NotifyToListeners(new ModelPropertyDifference.Scalar("ActiveFriendAddress", null));
                NotifyToListeners(new ModelPropertyDifference.Scalar("ActiveFriendDescription", null));
            }
            else
            {
                _activeFriendName = value.ToUpper();
                NotifyToListeners(new ModelPropertyDifference.Scalar("ActiveFriendName", value));
                NotifyToListeners(new ModelPropertyDifference.Scalar("ActiveFriendAddress", _friends[_activeFriendName].Address));
                NotifyToListeners(new ModelPropertyDifference.Scalar("ActiveFriendDescription", _friends[value.ToUpper()].Description));
            }
        });
    }
    ...
    #region privateメンバ
    readonly Dictionary<string, FriendProperties> _friends = new()
    {
        {"ALICE", new FriendProperties("tokyo", "Alice is a software engineer.")},
        {"BOB", new FriendProperties("saitama", "Bob is a designer.")},
        {"CHARLIE", new FriendProperties("kanagawa", "Charlie is a product manager.")}
    };
    string? _activeFriendName = null;
